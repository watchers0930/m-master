...
export function buildTopicRecommendations(params: {
  ...
  const keywordSuffix =
    params.sourceAnalysis.keywordHints.length > 0
      ? `: ${params.sourceAnalysis.keywordHints.slice(0, 3).join(", ")}`
      : "";

  return [
    {
      title: `${params.projectName}를 처음 이해하는 사람을 위한 핵심 가이드${keywordSuffix}`,
      intentType: "search",
      score: 9.4,
      rationale: `${sourceHint} 빠르게 이해시키는 입문형 주제로, 검색과 블로그 원문 생성에 가장 안정적이다.`,
    },
    {
      title: `${params.projectName}가 해결하는 문제와 실제 활용 흐름`,
      intentType: "branding",
      score: 8.9,
      rationale: `브랜드 가치와 사용 흐름을 함께 설명할 수 있어 블로그 본문과 인스타그램 카드 요약으로 동시에 확장하기 쉽다.`,
    },
    {
      title: `${params.projectName} 도입 전 반드시 확인할 체크리스트`,
