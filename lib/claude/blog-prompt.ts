...
본문에 정확히 \`[이미지: 구체적인 한국어 설명]\` 형식의 마커를 **반드시 10~15개** 삽입하세요.
이 규칙을 어기면 콘텐츠 전체가 무효 처리됩니다.
네이버 블로그 상위 노출에는 이미지 10장 이상이 필수이며, 최적값은 15장입니다.
...
- 본문에 마커가 0~9개 (반드시 10개 이상)
- 이모지나 ASCII art로 마커 대체
- 한 줄에 텍스트와 마커 혼합
- 이미지 2개 연속 배치 (반드시 텍스트 2~3줄 이상 사이에 둘 것)

### 필수 배치 위치 — "글 → 이미지 → 글 → 이미지" 교차 구조 (합산 10~15개)
1. 도입부(첫 문단 직후) — **1개**
2. 각 H2 섹션 시작 직후 — **각 1개씩** (H2가 7개면 7개)
3. 각 H2 섹션 본문 중간(2~3문단 후) — **추가 1개씩** (내용이 긴 H2에만)
4. 결론부 직전(마지막 H2 안) — **1개**
5. FAQ 섹션 질문 사이 — **1~2개**
...
### A. 이미지 마커 (가장 자주 누락됨)
- 본문에서 \`[이미지:\` 문자열 개수 세기 → **반드시 10~15개**
- 부족하면 H2 본문 중간과 FAQ 질문 사이에 추가
